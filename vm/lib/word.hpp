...
template <unsigned MaxBit> class Value {
...
public:
  ...
  template <unsigned M> explicit Value(Value<M> const &other) {
  ...
  constexpr std::byte hi() const noexcept { return m_data[1]; }
  constexpr std::byte lo() const noexcept { return m_data[0]; }
};
